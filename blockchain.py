...
class Blockchain:
	...
	def load_data(self):
		try:
			with open(BLOCKCHAIN_FILE.format(self.node_id), mode='r') as f:
	...
	def save_data(self):
		try:
			with open(BLOCKCHAIN_FILE.format(self.node_id), mode='w') as f:
			...
			try:
				...
				node_chain = [Block(block['index'], block['previous_hash'], 
					[Transaction(tx['sender'], tx['recipient'], tx['signature'], tx['amount']) 
						for tx in block['transactions']], block['proof'], block['timestamp']) 
